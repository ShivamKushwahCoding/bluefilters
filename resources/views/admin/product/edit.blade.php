        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
                    ...
                    <textarea class="form-control" style="height:50px" name="description" placeholder="description">{{ isset($product->description) ? $product->description : old('description')  }}</textarea>
                </div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                ...
                @if (isset($product->description))
                    <img class="img-fluid img-thumbnail" src="{{ asset('product/images/' . $product->image) }}">
                @endif
            </div>
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
                    ...
                    <select name="category_id" id="category-dropdown" class="form-control"
                        ...
                        @if (!empty($category))
                            ...
                            @foreach ($category as $value)
                                @if (isset($product) && $value->id == $product->category_id)
                    ...
                    <select id="sub-category-dropdown" name="sub_category_id" class="form-control"
                        ...
                        @if (!empty($sub_category))
                            @foreach ($sub_category as $value)
                                @if (isset($product) && $value->id == $product->sub_category_id)
                                    <option value="{{ $value->id }}">{{ $value->name }}</option>
                                @else
                                    <option value="{{ $value->id }}">{{ $value->name }}</option>
                                @endif
